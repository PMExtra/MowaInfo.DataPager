...
{
    /// <summary>
    ///     A factory to create dynamic classes, based on
    ///     <see href="http://stackoverflow.com/questions/29413942/c-sharp-anonymous-object-with-properties-from-dictionary" />
    ///     .
    /// </summary>
    public static class DynamicClassFactory
    {
        ...
        private static readonly CustomAttributeBuilder CompilerGeneratedAttributeBuilder =
            new CustomAttributeBuilder(typeof(CompilerGeneratedAttribute).GetConstructor(EmptyTypes), new object[0]);
        ...
        private static readonly MethodInfo ObjectToString =
            typeof(object).GetMethod("ToString", BindingFlags.Instance | BindingFlags.Public);
        ...
        private static readonly MethodInfo EqualityComparerDefault =
        ...
        /// <summary>
        ///     The CreateType method creates a new data class with a given set of public properties and returns the System.Type
        ///     object for the newly created class. If a data class with an identical sequence of properties has already been
        ///     created, the System.Type object for this class is returned.
        ///     Data classes implement private instance variables and read/write property accessors for the specified
        ///     properties.Data classes also override the Equals and GetHashCode members to implement by-value equality.
        ///     Data classes are created in an in-memory assembly in the current application domain. All data classes inherit from
        ///     <see cref="DynamicClass" /> and are given automatically generated names that should be considered private (the
        ///     names will be unique within the application domain but not across multiple invocations of the application). Note
        ///     that once created, a data class stays in memory for the lifetime of the current application domain. There is
        ///     currently no way to unload a dynamically created data class.
        ///     The dynamic expression parser uses the CreateClass methods to generate classes from data object initializers. This
        ///     feature in turn is often used with the dynamic Select method to create projections.
        /// </summary>
        /// <param name="properties">The DynamicProperties</param>
        /// <param name="createParameterCtor">
        ///     Create a constructor with parameters. Default set to true. Note that for
        ///     Linq-to-Database objects, this needs to be set to false.
        /// </param>
            ...
            if (!GeneratedTypes.TryGetValue(key, out type))
            {
                lock (GeneratedTypes)
                {
                    if (!GeneratedTypes.TryGetValue(key, out type))
                    {
                        ...
                        var name = names.Length != 0
                            ? $"<>f__AnonymousType{index}`{names.Length}"
                            : $"<>f__AnonymousType{index}";

                        var tb = ModuleBuilder.DefineType(name,
                            TypeAttributes.AnsiClass | TypeAttributes.Public | TypeAttributes.Class |
                            TypeAttributes.AutoLayout | TypeAttributes.BeforeFieldInit, typeof(DynamicClass));
                        ...
                        {
                            // field
                            fields[i] = tb.DefineField($"<{names[i]}>i__Field", generics[i].AsType(),
                                FieldAttributes.Private | FieldAttributes.InitOnly);
                            fields[i].SetCustomAttribute(DebuggerBrowsableAttributeBuilder);

                            var property = tb.DefineProperty(names[i], PropertyAttributes.None,
                                CallingConventions.HasThis, generics[i].AsType(), EmptyTypes);

                            // getter
                            var getter = tb.DefineMethod($"get_{names[i]}",
                                MethodAttributes.Public | MethodAttributes.HideBySig | MethodAttributes.SpecialName,
                                CallingConventions.HasThis, generics[i].AsType(), null);
                        ...
                        var toString = tb.DefineMethod("ToString",
                            ...
                            CallingConventions.HasThis, typeof(string), EmptyTypes);
                        ...
                        var getHashCode = tb.DefineMethod("GetHashCode",
                            MethodAttributes.Public | MethodAttributes.Virtual | MethodAttributes.HideBySig,
                            CallingConventions.HasThis, typeof(int), EmptyTypes);
                        ...
                        for (var i = 0; i < names.Length; i++)
                        {
                            ...
                            // GetHashCode();
                            var equalityComparerTGetHashCode =
                        ...
                        {
                            // .ctor default
                            var constructorDef = tb.DefineConstructor(
                            ...
                            // .ctor with params
                            var constructor = tb.DefineConstructor(MethodAttributes.Public | MethodAttributes.HideBySig,
                                CallingConventions.HasThis, generics.Select(p => p.AsType()).ToArray());
                        ...
                        type = tb.CreateTypeInfo().GetType();
        ...
        /// <summary>
        ///     Generates the key.
        ///     Anonymous classes are generics based. The generic classes are distinguished by number of parameters and name of
        ///     parameters. The specific types of the parameters are the generic arguments.
        /// </summary>
        ...
        private static string GenerateKey(IEnumerable<DynamicProperty> dynamicProperties, bool createParameterCtor)
        {
            ...
            return string.Format("{0}_{1}",
                string.Join("|", dynamicProperties.Select(p => Escape(p.Name) + "~" + p.Type.FullName).ToArray()),
                createParameterCtor ? "c" : string.Empty);
        }
